...
function WorkingHistory() {
  ...
  const getStrokeDashOffset = (effectiveTime) => {
    const hours = parseFloat(effectiveTime.split(":")[0]);
    if (hours > 9) {
      return (226 * (100 - 90)) / 100;
    } else {
      return (226 * (100 - (hours / 9) * 100)) / 100;
    }
  };
  ...
  const sortedData = [...workingHistoryData].sort((a, b) => {
    ...
    switch (sortKey) {
      ...
      case "departureTime":
        aValue =
          a.departureTime === "Still in office"
            ? Infinity
            : new Date(`1970-01-01T${a.departureTime}`);
        bValue =
          b.departureTime === "Still in office"
            ? Infinity
            : new Date(`1970-01-01T${b.departureTime}`);
        break;
      case "effectiveTime":
    ...
    <div className="bg-white p-4  rounded-lg shadow-md w-full border-[1px]" style={{marginTop:"10px"}}>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="absolute right-20 -mt-10">
          <select
            className="block appearance-none w-full py-2 px-6 border border-gray-300 bg-white text-gray-700 text-xs rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-left"
            style={{ marginRight: "-9px", marginLeft: "22px" }}
          >
          ...
          <thead>
            <tr>
              ...
              <th className="p-2 text-left text-gray-500 bg-gray-100 text-xs">
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <FaSortDown
                      className={`inline-block -mt-[10px] ${
                        sortKey === "arrivalTime" && sortOrder === "desc"
                          ? "text-blue-600"
                          : "text-gray-400"
                      }`}
                    />
              ...
              <th className="p-2 text-left text-gray-500 bg-gray-100 text-xs">
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <FaSortDown
                      className={`inline-block -mt-[10px] ${
                        sortKey === "departureTime" && sortOrder === "desc"
                          ? "text-blue-600"
                          : "text-gray-400"
                      }`}
                    />
              ...
              <th className="p-2 text-left text-gray-500 bg-gray-100 text-xs">
                <div
                  className="flex justify-between items-center cursor-pointer"
                  onClick={() => handleSort("effectiveTime")}
                >
                  Effective time
                  <div className="flex flex-col">
                    <FaSortUp
                      className={`inline mt-2 ${
                        sortKey === "effectiveTime" && sortOrder === "asc"
                          ? "text-blue-600"
                          : "text-gray-400"
                      }`}
                    />
                    <FaSortDown
                      className={`inline-block -mt-[10px] ${
                        sortKey === "effectiveTime" && sortOrder === "desc"
                          ? "text-blue-600"
                          : "text-gray-400"
                      }`}
                    />
                ...
                return (
                  <tr key={index} className="hover:bg-gray-200">
                    <td className="p-2 text-left flex items-center mt-1 text-xs">
                      <div
                        className={`w-6 h-6 flex items-center justify-center rounded-full bg-gray-300 text-gray-700 text-xs mr-2`}
                      >
                        {isToday ? "T" : entry.date.split("/")[0]}
                      </div>

                      {isToday ? (
                        <span className="text-blue-600">Today's</span>
                      ) : (
                        entry.date
                      )}
                    </td>
                    ...
                    <td className="p-2 text-left text-xs">
                      <div className="flex items-center justify-around">
                        ...
                        <svg className="w-8 h-6" viewBox="0 0 80 80">
                          <circle
                          ...
                          />
                          {parseFloat(entry.effectiveTime.split(":")[0]) >
                            9 && (
                            <circle
                              className="text-orange-500"
                              strokeWidth="10"
                              strokeDasharray="226"
                              strokeDashoffset={(226 * (100 - 100)) / 100}
                              strokeLinecap="round"
                              stroke="currentColor"
                              fill="transparent"
                              r="36"
